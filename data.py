...
class Vocab(object):
  ...
  def __init__(self, vocab_file, max_size):
    ...
    if FLAGS.use_glove:
        print 'INFO: use pre-train glove, loading the glove ...'
        word2glove = {}
        with open(FLAGS.glove_path, 'r') as glove_f:
          for line in glove_f:
            line = line.split()
            if line[0].lower() == 'pad':
              word2glove[PAD_TOKEN] = np.asarray(line[1:], dtype='float')
            elif line[0].lower() == 'sos':
              word2glove[DECODING_START]= np.asarray(line[1:], dtype='float')
            elif line[0].lower() == 'eos':
              word2glove[DECODING_END]= np.asarray(line[1:], dtype='float')
            elif line[0].lower() == 'unk':
              word2glove[UNKNOWN_TOKEN]= np.asarray(line[1:], dtype='float')
            else:
              word2glove[line[0].lower()] = np.asarray(line[1:], dtype='float')
        ...
        for w in [UNKNOWN_TOKEN, PAD_TOKEN, DECODING_START, DECODING_END]:
          ...
          self.id_to_word[self.count] = w.decode('utf-8')
        ...
        with open(vocab_file, 'r') as vocab_f:
          for line in vocab_f:
            ...
            if word2glove.has_key(pieces[0]):
              self.word_to_id[pieces[0]] = self.count
              self.id_to_word[self.count] = pieces[0].decode('utf-8')
  ...
  def _word2glove(self, word):
    if word_id not in self.word_to_glove:
